...
func _input(event):
	...
	if event is InputEventKey:
		if event.is_command_or_control_pressed() && event.pressed && not event.echo:
			...
			# --- FIX START ---
			# If the key is a period/dot, force it via JavaScript
			if event.pressed and (event.keycode == KEY_PERIOD or event.keycode == 46):
				# This JavaScript command inserts a dot at the current cursor position
				current_browser.execute_javascript("document.execCommand('insertText', false, '.');")
				return # Stop here so we don't send the confusing key signal to CEF
			# --- FIX END ---
			
			# For all other keys, send normally
